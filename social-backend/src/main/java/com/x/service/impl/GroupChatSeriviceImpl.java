...
public class GroupChatSeriviceImpl implements GroupChatSerivice {
    ...
    public GroupSearchVO searchGroup(String account) {
        ...
        if(groupChat == null){
            ...
            //判断是否是群主
            int countt=groupChatMapper.getCountByGroupIdAndUserId(groupChat.getId(),BaseContext.getCurrentId());
            if(countt>0){
            ...
            //判断是否已经加入
            int count=groupMemberMapper.getCountByGroupIdAndUserId(groupChat.getId(), BaseContext.getCurrentId());
            if(count>0){
                return GroupSearchVO.builder()
                        .id(groupChat.getId())
                        .groupAvatar(groupChat.getGroupAvatar())
