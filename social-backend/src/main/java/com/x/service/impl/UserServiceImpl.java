...
public class UserServiceImpl implements UserService {
    ...
    public UserSearchVO searchByAccount(String account) {
        ...
        Integer type=1;
        Notification notification= notificationMapper.getByIds(BaseContext.getCurrentId(),receiverId,type);
        if(notification!=null) {
            userSearchVO.setStatus("applied");
            return userSearchVO;
        }
        //找到，但是是本人
        if(BaseContext.getCurrentId().equals(receiverId)) {
            userSearchVO.setStatus("self");
            return userSearchVO;
        }
        //找到,但是是好友了
        Friend friend= friendMapper.getByIds(BaseContext.getCurrentId(),receiverId);
        if(friend!=null) {
