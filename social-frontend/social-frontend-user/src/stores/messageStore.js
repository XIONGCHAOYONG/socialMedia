...
export const useMessageStore = defineStore('message', () => {
  ...
  const addUserInfoToMessages = async (messages) => {
    ...
    for (const message of messages) {
      // 获取消息中非当前用户的ID
      const otherUserId = message.senderId === loginStore.getUser.id
        ? message.receiverId
        : message.senderId;
  ...
  const fetchMesListAll = async () => {
    const res = await PrivateMessage.getMesListAll(loginStore.getUser.id);
    if (res.data.code === 1) {
      // 为消息添加用户信息
  ...
  const fetchMesList = async () => {
    const res = await PrivateMessage.getMesList(loginStore.getUser.id);
