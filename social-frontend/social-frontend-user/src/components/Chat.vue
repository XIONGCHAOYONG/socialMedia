<template>
  <div class="chat">
    <div class="chatMes-container">
      <div class="change">
        <div :class="['switch-btn', 'privateMes', { active: isPrivateChat }]" @click="toPrivateMes">私信</div>
        <div :class="['switch-btn', 'groupMes', { active: isGroupChat }]" @click="toGroupChat">群聊</div>
      </div>

      <el-scrollbar max-height="calc(100vh - 100px)" v-if="isPrivateChat">
        <el-timeline>
          <el-timeline-item v-for="(message, index) in messageStore.getMesList" :key="index"
            :timestamp="formatDate(message.createdAt)" placement="top"
            @click.native="messageStore.setCurChatUserAndIndex(message.senderId === loginStore.getUser.id ? message.receiverId : message.senderId, index)">
            <el-card :class="{
              ...
              <div class="message">
                <div class="avatar">
                  <Avatar :avatar="message.avatar" :username="message.username || '用户'"
                    :style="{ width: '50px', height: '50px', fontSize: '20px' }" />
                </div>
                <div class="content">
                  <div class="info">
                    <span class="name">{{ message.username }}</span>
                    <span class="time">{{ formatDate(message.createdAt, 'time') }}</span>
                  </div>
                  <div class="text">{{ message.senderId === loginStore.getUser.id ? '' : message.username + ': ' }}{{
                    message.content }}</div>
                </div>
              </div>
            </el-card>
      ...
      <el-scrollbar max-height="calc(100vh - 100px)" v-if="isGroupChat">
        <el-timeline>
          <el-timeline-item v-for="(message, index) in groupMessageStore.getMesList" :key="index"
            :timestamp="formatDate(message.messageTime)" placement="top"
            @click.native="groupMessageStore.setCurMesListIndex(index)">
            <el-card :class="{
              // 'unread-message': !message.isRead,
              'selected-message': index === groupMessageStore.getCurMesListIndex
            }">
              <div class="message">
                <div class="avatar">
                ...
                <div class="content">
                  <div class="info">
                    ...
                    <span class="time">{{ formatDate(message.messageTime, 'time') }}</span>
                  </div>
                  <div class="text">{{ message.senderId === loginStore.getUser.id ?
                    '' : message.senderName + ': ' }}{{
                      message.content
                    }}</div>
                </div>
    ...
    <div class="chatContainer" v-if="isPrivateChat">
      ...
      <div class="chat-messages-area" :style="chatAreaStyle">
        <el-scrollbar ref="messageScrollbar" max-height="calc(100vh - 200px)">
          <div class="chat-messages">
            <div v-for="(message, index) in messageStore.getCurMesList" :key="index" class="chat-message"
              :class="{ 'self-message': message.senderId === loginStore.getUser.id }">
              <div class="message-bubble">
                <div class="message-avatar">
                  <Avatar
                    :avatar="message.senderId === loginStore.getUser.id ? loginStore.getUser.avatar : messageStore.getCurChatUser?.avatar"
                    :username="message.senderId === loginStore.getUser.id ? loginStore.getUser.username : messageStore.getCurChatUser?.username || '用户'" />
                </div>
                <div class="message-content">
                  <div class="message-time">{{ formatDate(message.createdAt, 'datetime') }}</div>
      ...
      <div class="chat-input-area">
        <el-row align="middle">
          <el-col :span="21">
            <el-input v-model="messageInput" :autosize="{ minRows: 1, maxRows: 4 }" type="textarea"
              placeholder="请输入消息..." resize="none" @keyup.enter.native="sendMessage" />
          </el-col>
    ...
    <div class="chatContainer" v-if="isGroupChat">
      ...
      <div class="chat-messages-area" :style="chatAreaStyle">
        <el-scrollbar ref="messageScrollbar" max-height="calc(100vh - 200px)">
          <div class="chat-messages">
            <div v-for="(message, index) in groupMessageStore.getCurMesList" :key="index" class="chat-message"
              :class="{ 'self-message': message.senderId === loginStore.getUser.id }">
              <div class="message-bubble">
                <div class="message-avatar">
                  <Avatar
                    :avatar="message.senderId === loginStore.getUser.id ? loginStore.getUser.avatar : message.senderAvatar"
                    :username="message.senderId === loginStore.getUser.id ? loginStore.getUser.username : message.senderUsername || '用户'" />
                </div>
                <div class="message-content">
                  <div class="message-time">{{ formatDate(message.messageTime, 'datetime') }}</div>
                  <div class="message-text">{{ message.content }}</div>
                </div>
              </div>
            </div>
      ...
      <div class="chat-input-area">
        <el-row align="middle">
          <el-col :span="21">
            <el-input v-model="messageInput" :autosize="{ minRows: 1, maxRows: 4 }" type="textarea"
              placeholder="请输入消息..." resize="none" @keyup.enter.native="sendGroupMessage" />
          </el-col>
          <el-col :span="3" style="text-align: right;">
...
const chatAreaStyle = computed(() => {
  if (loginStore.getUser?.cover && isBgImageEnabled.value) {
    return {
      '--chat-bg-image': `url(${loginStore.getUser.cover})`,
    };
  }
  return {};
});
...
const sendMessage = async () => {
  ...
  const mes = {
    ...newMessage,
    avatar: loginStore.getUser.avatar,
...
onMounted(async () => {
  ...
  instantJumpToBottom();
  // 连接WebSocket
  wsChatStore.connect(loginStore.getUser.id);


  groupMessageStore.fetchMesList();
  groupMessageStore.fetchMesListAll();
 ...
 */
watch([() => messageStore.getCurChatUser, () => messageStore.getMesListAll,
() => groupMessageStore.getCurGroup, groupMessageStore.getMesListAll], () => {
