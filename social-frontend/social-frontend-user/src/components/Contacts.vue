<template>
  <div class="contacts">
    ...
    <div class="notice">
      <div class="notice-header">
        <h3>通知 <span style="font-size: 14px; color: #909399;">({{ (receivedNotificationList?.length || 0) +
          (sentNotificationList?.length || 0) }})</span></h3>
        <el-badge :value="unhandledNoticeCount" :hidden="unhandledNoticeCount === 0">
          <el-icon>
            <Bell />
          </el-icon>
        </el-badge>
      ...
      <div class="notice-content">
        <el-scrollbar height="calc(100vh - 190px)" class="custom-scrollbar">
          <div v-if="receivedNotificationList.length === 0 && sentNotificationList.length === 0" class="empty-notice">
          ...
          <div v-else class="notice-list">
            ...
            <!-- //receiver -->
            RECEIVE
            <div v-for="notice in receivedNotificationList" :key="notice.id" class="notice-item">
              <div class="notice-avatar">
                <el-avatar :size="40" :src="notice.avatar" />
              </div>
              <div class="notice-info">
                <div class="notice-title">
                  {{ notice.type === 1 ? '好友申请' : notice.type === 2 ? '群聊邀请' : '通知' }}
                </div>
                <div class="notice-content-text">{{ getNoticeContent(notice) }}</div>
                <div class="notice-time">{{ formatDate(notice.createdAt, 'datetime') }}</div>
              </div>
              <div class="notice-actions">
                <!-- 接收方 -->
                <template v-if="loginStore.user.id === notice.userId">
                  <el-button v-if="notice.status === 0" type="primary" size="small"
                    @click="handleAcceptNotice(notice)">接受</el-button>
                  <el-button v-if="notice.status === 0" type="danger" size="small"
                    @click="handleRejectNotice(notice)">拒绝</el-button>
                  <el-button v-if="notice.status === 1" type="success" size="small" disabled>已同意</el-button>
                  <el-button v-if="notice.status === 2" type="info" size="small" disabled>已拒绝</el-button>
                </template>
                <!-- 发送方 -->
                <template v-else-if="loginStore.user.id === notice.senderId">
                  <el-button v-if="notice.status === 0" type="warning" size="small" disabled>申请中</el-button>
                  <el-button v-if="notice.status === 1" type="success" size="small" disabled>对方已同意</el-button>
                  <el-button v-if="notice.status === 2" type="info" size="small" disabled>对方已拒绝</el-button>
                </template>
              </div>
            </div>
            ...
            SEND
            <div v-for="notice in sentNotificationList" :key="notice.id" class="notice-item">
              <div class="notice-avatar">
                <el-avatar :size="40" :src="notice.avatar" />
              </div>
              <div class="notice-info">
    ...
    <div class="friendList">
      <div class="friend-header">
        <h3>好友列表 <span style="font-size: 14px; color: #909399;">({{ friendList?.length }})</span></h3>
        <div class="search-add-container">
          ...
          <el-input v-model="searchKeyword" placeholder="搜索好友" prefix-icon="Search" clearable style="width: 200px;" />
        </div>
      </div>
      ...
      <div class="friend-content">
        <el-scrollbar height="calc(100vh - 190px)" class="custom-scrollbar">
          ...
          <div v-else class="friend-list">
            <div v-for="(friend, index) in filteredFriendList" :key="friend.id" class="friend-item">
              <div class="friend-card-content" :class="{ 'reverse': index % 2 === 1 }">
                <!-- 头像区域 -->
                <div class="avatar-section">
                  <div class="avatar-container">
                    <Avatar :avatar="friend.avatar" :username="friend.username || '用户'"
                ...
                <div class="info-section">
                  <el-descriptions direction="vertical" border :column="2">
                    <el-descriptions-item label="名称">{{ friend.username }}</el-descriptions-item>
                    <el-descriptions-item label="性别">{{ friend.gender === 1 ? "男" : friend.gender === 2 ? "女" : "保密"
                      }}</el-descriptions-item>
                    <el-descriptions-item label="地址">{{ friend.region || '未设置' }}</el-descriptions-item>
                    <el-descriptions-item label="加好友时间">{{ formatDate(friend.createdAt, 'datetime')
                    }}</el-descriptions-item>
  ...
  <!-- 添加好友弹窗 -->
  <el-dialog v-model="showAddFriendDialog" title="添加好友" width="500px" @close="handleCloseDialog">
    <el-form :model="addFriendForm" label-width="80px" @submit.prevent>
      <el-form-item label="账号">
        <el-input v-model="addFriendForm.account" placeholder="请输入要查找的账号" clearable @keyup.enter="handleSearchUser" />
      </el-form-item>
    </el-form>

    <!-- 查找结果 -->
    <div v-if="searchResult && searchResult.status !== 'failed'" class="search-result">
      ...
      <div class="user-info">
        ...
        <div class="user-details">
          ...
          <div class="signature">{{ searchResult.signature || '这个人很懒，什么都没有留下~' }}</div>
          <div class="user-actions">
            <el-button v-if="searchResult && searchResult.status === 'pending'" type="primary"
              @click="handleSendFriendRequest()">
              发送好友申请
            </el-button>
            <el-button v-else-if="searchResult && searchResult.status === 'applied'" type="success" disabled>
              已申请
            </el-button>
            <el-button v-else-if="searchResult && searchResult.status === 'friend'" type="info" disabled>
              已是好友
            </el-button>
            <el-button v-else-if="searchResult && searchResult.status === 'self'" type="info" disabled>
              本人
            </el-button>
          </div>
        </div>
  ...
  <!-- 群聊管理弹窗 -->
  <el-dialog v-model="showGroupManageDialog" title="群聊管理" width="80%" @close="handleCloseGroupDialog">
        ...
        <el-tabs v-model="activeGroupTab" class="group-tabs">
          <el-tab-pane label="我创建的群聊" name="created">
            <div class="group-list">
              ...
              <div v-else class="group-grid">
                <div v-for="group in myCreatedGroups" :key="group.id" class="group-card">
                  <el-button class="dissolve-btn" type="danger" size="small"
                  ...
                  <div class="group-card-header">
                    <div class="group-avatar">
                      <img :src="group.groupAvatar || '/src/assets/img/avatar.png'" alt="群聊头像" />
                    </div>
                  ...
                  <div class="group-card-content">
                    <p class="group-time">创建时间：{{ formatDate(group.createdAt, 'datetime') }}</p>
                  </div>
                  <div class="group-card-actions">
                    <el-button type="primary" size="small" @click="handleGroupChat(group)">
                      <el-icon>
                        <ChatDotRound />
                      </el-icon>
                      进入群聊
                    </el-button>
                    <el-button type="success" size="small" @click="handleGroupCommand({ action: 'edit', group })">
                      <el-icon>
                        <Edit />
                      </el-icon>
                      编辑
                    </el-button>
                    <el-button type="warning" size="small" @click="handleGroupCommand({ action: 'members', group })">
                      <el-icon>
                        <User />
                      </el-icon>
                      成员
                    </el-button>
                    <el-button class="invite-btn" size="small" @click="handleGroupCommand({ action: 'invite', group })">
                      <el-icon>
                        <Plus />
                      </el-icon>
                      邀请
          ...
          <el-tab-pane label="我加入的群聊" name="joined">
            <div class="group-list">
              ...
              <div v-else class="group-grid">
                <div v-for="group in myJoinedGroups" :key="group.id" class="group-card">
                  <div class="group-card-header">
                    <div class="group-avatar">
                      <img :src="group.groupAvatar || '/src/assets/img/avatar.png'" alt="群聊头像" />
                    </div>
                  ...
                  <div class="group-card-actions">
                    ...
                    <el-button type="danger" size="small" @click="handleGroupCommand({ action: 'leave', group })">
                      <el-icon>
                        <Close />
                      </el-icon>
                      退出
  ...
  <!-- 创建群聊弹窗 -->
  <el-dialog v-model="showCreateGroupDialog" title="创建群聊" width="600px" @close="handleCloseCreateGroupDialog">
    <div class="create-group-container">
      <div class="create-group-form">
        <el-form :model="createGroupForm" :rules="createGroupRules" ref="createGroupFormRef" label-width="80px">
          <el-form-item label="群聊头像">
            <div class="avatar-upload-container">

              <el-upload class="avatar-uploader" :show-file-list="false" :before-upload="beforeUpload"
                :on-success="handleAvatarUploadSuccesss" accept="image/*" :action="`${uploadUrl}/user/upload`"
                :headers="headers">
                <img v-if="createGroupForm.groupAvatar" :src="createGroupForm.groupAvatar" class="avatar-preview" />
          ...
          <el-form-item label="群聊名称" prop="name">
            <el-input v-model="createGroupForm.name" placeholder="请输入群聊名称" maxlength="20" show-word-limit />
          </el-form-item>
          <el-form-item label="群号" prop="groupNumber">
            <el-input v-model="createGroupForm.groupNumber" placeholder="请输入群号" maxlength="10" show-word-limit />
            ...
            <div class="member-selector">
              <div class="friend-selection-list">
                <el-checkbox-group v-model="createGroupForm.selectedMembers">
                  <div v-for="friend in friendList" :key="friend.friendId" class="friend-selection-item">
                    <el-checkbox :label="friend.friendId">
                      <div class="friend-selection-info">
                        <el-avatar :size="30" :src="friend.avatar || '../assets/img/avatar.png'" />
                        <span class="friend-selection-name">{{ friend.username }}</span>
                      </div>
                    </el-checkbox>
                  </div>
                </el-checkbox-group>
  ...
  <el-dialog v-model="showMembersDialog" title="群成员管理" width="700px">
    <div class="member-container">
      <div class="members-header">
        <h4>{{ currentGroup?.groupName }} - 成员列表 ({{ groupMembers?.length }}人)</h4>

      </div>
      <div class="member-list">
        <div v-for="member in groupMembers" :key="member.id" class="member-item">
          <div class="member-info">
            <el-avatar :size="40" :src="member.avatar || '../assets/img/avatar.png'" />
            <div class="member-details">
              <div class="member-name">
                {{ member.username }}
                <el-tag v-if="member.id === currentGroup?.creatorId" size="small" type="success">群主</el-tag>
                <el-tag v-else-if="member.id === loginStore.getUser.id" size="small" type="info">我</el-tag>
              </div>
              <div class="member-meta">
                <div class="member-time">加入时间：{{ formatDate(member.joinedAt, 'datetime') }}</div>
                <div class="member-account">账号：{{ member.account }}</div>
              </div>
            </div>
          </div>
          <div class="member-actions">
            <el-button v-if="isGroupCreator && member.id !== loginStore.getUser.id" type="danger" size="small"
  ...
  <el-dialog v-model="showInviteDialog" title="邀请成员" width="600px">
    <div class="invite-container">
      <div class="invite-user-list">
        <div v-for="user in usersForInvite" :key="user.id" class="invite-user-item">
          <div class="invite-user-info">
            <el-avatar :size="40" :src="user.avatar || '../assets/img/avatar.png'" />
          ...
          <el-button v-if="!user.invited" class="invite-btn" size="small" @click="handleInviteUser(user)">
            <el-icon>
              <Plus />
            </el-icon>
            邀请
  ...
  <el-dialog v-model="showEditGroupDialog" title="编辑群信息" width="500px">
    <el-form :model="editGroupForm" :rules="editGroupRules" ref="editGroupFormRef" label-width="80px">
      <el-form-item label="群聊头像">
        <div class="avatar-upload-container">
          <el-upload class="avatar-uploader" :show-file-list="false" :before-upload="beforeUpload"
            :on-success="handleAvatarUploadSuccess" accept="image/*" :action="`${uploadUrl}/user/upload`"
            :headers="headers">
            <img v-if="editGroupForm.groupAvatar" :src="editGroupForm.groupAvatar" class="avatar-preview" />
            <el-icon v-else class="avatar-uploader-icon">
              <Plus />
            </el-icon>
          </el-upload>
      ...
      <el-form-item label="群聊名称" prop="name">
        <el-input v-model="editGroupForm.name" placeholder="请输入群聊名称" maxlength="20" show-word-limit />
      </el-form-item>
    </el-form>
    <template #footer>
      <span class="dialog-footer">
  ...
  <!-- 加入群聊弹窗 -->
  <el-dialog v-model="showJoinGroupDialog" title="加入群聊" width="600px" @close="handleCloseJoinGroupDialog"
    class="join-group-dialog">
    <div class="join-group-container">
      <el-form :model="joinGroupForm" label-width="80px" @submit.prevent>
        <el-form-item label="群号">
          <el-input v-model="joinGroupForm.searchKeyword" placeholder="请输入群号" clearable
      ...
      <div v-if="groupSearchResult" class="search-result-section">
        ...
        <div class="group-search-item">
          <div class="group-search-info">
            <el-avatar :size="50" :src="groupSearchResult.groupAvatar || '../assets/img/avatar.png'" />
            <div class="group-search-details">
              <div class="group-search-name">{{ groupSearchResult.groupName }}</div>
              <div class="group-search-number">群号：{{ groupSearchResult.groupAccount }}</div>
              <div class="group-search-members">{{ groupSearchResult.memberCount }}人</div>
              <div class="group-search-creator">群主：{{ groupSearchResult.creatorName }}</div>
            </div>
          </div>
          <div class="group-search-actions">
            <el-button v-if="groupSearchResult.status === 'notJoined'" type="primary" size="small"
              @click="handleJoinGroup(groupSearchResult)">
              申请加入
            </el-button>
            <el-button v-else-if="groupSearchResult.status === 'pending'" type="warning" size="small" disabled>
              申请中
            </el-button>
            <el-button v-else-if="groupSearchResult.status === 'joined'" type="success" size="small" disabled>
              已是成员
            </el-button>
            <el-button v-else-if="groupSearchResult.status === 'created'" type="info" size="small" disabled>
              我创建的
            </el-button>
          </div>
        </div>
      ...
      <!-- 未找到结果 -->
      <div v-else-if="hasGroupSearched && (!groupSearchResult || groupSearchResult.length === 0)"
...
import { Bell, User, Search, ChatDotRound, Delete, Picture, Plus, CircleCloseFilled, MoreFilled, Edit, View, Close, UserFilled } from '@element-plus/icons-vue'
...
const isGroupCreator = computed(() => {
  ...
  return currentGroup.value.creatorId === loginStore.getUser.id
})
...
const handleGroupCommand = async ({ action, group }) => {
  ...
  switch (action) {
    ...
    case 'invite':
      ...
      await groupChatStore.fetchGroupMembers(currentGroup.value.id)
      groupMembers.value = groupChatStore.getGroupMembers
      const invitableFriends = getInvitableFriends(friendList.value, groupMembers.value)
      usersForInvite.value = invitableFriends.map(friend => ({ ...friend, invited: false }))
      break
    case 'dissolve':
...
const handleCreateGroup = async () => {
  ...
  try {
    ...
    const res = await GroupChat.createGroup({
      ...
      creatorid: loginStore.getUser.id,
...
const handleDissolveGroup = async (group) => {
  try {
    ...
    // 这里调用解散群聊API
    const res = await GroupChat.dissolveGroup(
      group.id
    )
    if (res.data.code == 1) {
...
// 通知列表数据渲染
const receivedNotificationList = computed(() => notificationStore.getReceivedNotificationList || [])
const sentNotificationList = computed(() => notificationStore.getSentNotificationList || [])

// 1. 恢复friendList为computed写法
const friendList = computed(() => friendStore.getFriendList || [])

onMounted(async () => {
  // friendList.value = friendStore.getFriendList
  //确保好数据已加载
...
const filteredFriendList = computed(() => {
  ...
  return friendList.value.filter(friend =>
    ...
    friend.region?.toLowerCase().includes(searchKeyword.value.toLowerCase()) ||
    friend.signature?.toLowerCase().includes(searchKeyword.value.toLowerCase())
  )
})
  ...
  if (notice.type === 1) {
    ...
    if (res.data.code == 1) {
      ...
      if (ress.data.code == 1) {
        ...
        const content = loginStore.getUser.id === notice.senderId ? "你好,我是" + loginStore.getUser.username : "你好,我是" + notice.username;
        //添加打招呼的信息,发送方给接收方私信
  ...
  if (notice.type == 2) {
    ...
    if (res.data.code == 1) {
      ...
      if (ress.data.code == 1) {
        ...
        //主动刷新
        groupChatStore.fetchMyJoinedGroups()
        const content = "大家好,我是" + loginStore.getUser.username
        //添加打招呼的信息,发送方给接收方私信
        ...
        const mes = {
          ...
          senderId: loginStore.getUser.id,
          groupId: notice.groupId,
          senderAvatar: loginStore.getUser.avatar,
          senderName: loginStore.getUser.username,
          type: 'group',
...
const unhandledNoticeCount = computed(() => {
  return receivedNotificationList.value.filter(n => n.status === 0 && n.userId === myId).length +
    sentNotificationList.value.filter(n => n.status === 0 && n.senderId === myId).length;
})
...
const handleChat = (friend) => {
  ...
  messageStore.jumpFromContacts(loginStore.getUser.id, friend.friendId);
  //跳转到聊天页面
  router.push("/chat");
}
//点击朋友圈按钮，跳转
